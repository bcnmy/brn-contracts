...
library SmartWalletFactory {
    event SmartContractWalletDeployed(
        ...
        ISmartWallet indexed smartContractWallet
    );

    function deploySmartContractWallet(
        address _owner,
        address _implementation
    ) internal returns (ISmartWallet) {
        ISmartWallet wallet = ISmartWallet(
            Clones.cloneDeterministic(
                _implementation,
                smartContractWalletSalt(_owner)
            )
        );
    ...
    function getSmartContractWalletInstance(
        address _owner,
        address _implementation
    ) internal returns (ISmartWallet) {
        address scw = predictSmartContractWalletAddress(
            _owner,
            _implementation
        );
        uint256 scwCodeSize;
    ...
    function smartContractWalletSalt(
        address _owner
